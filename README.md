...
If you place a standard SPI DAC IC on the evaluation board you could even compare a delayed version of the ADC analog input with itself (address data would need to configured to be static and single byte). The point is that driving ADC conversion data out using the SPI bus format is useful.
